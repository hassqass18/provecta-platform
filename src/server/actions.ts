...
// ── Acquisition funnel (web server actions; mobile uses the matching API routes) ──

// Start a prospect: research → proposal generation are enqueued automatically.
export async function createProspectAction(formData: FormData) {
  ...
  revalidatePath("/admin/engagements");
  redirect(`/admin/engagements/${r.engagementId}`);
}

// (Re)generate the proposal body for an engagement, grounded on its research brief.
export async function generateProposalAction(formData: FormData) {
  const admin = await requireAdmin();
  const engagementId = String(formData.get("engagementId") ?? "");
  if (!engagementId) return;
  await generateProposalForEngagement(engagementId).catch(() => {});
  await audit(admin.id, "PROPOSAL_DRAFTED", "Engagement", engagementId, "regenerated by operator");
  revalidatePath(`/admin/engagements/${engagementId}`);
}

// Operator-gated proposal send (mints the public accept link + emails the prospect).
export async function sendProposalAction(formData: FormData) {
  const admin = await requireAdmin();
  const proposalId = String(formData.get("proposalId") ?? "");
  const engagementId = String(formData.get("engagementId") ?? "");
  if (!proposalId) return;
  await sendProposal(proposalId, admin.id);
  if (engagementId) revalidatePath(`/admin/engagements/${engagementId}`);
}

// Operator releases the reviewed engagement agreement for the client to sign.
export async function releaseContractAction(formData: FormData) {
  const admin = await requireAdmin();
  const envelopeId = String(formData.get("envelopeId") ?? "");
  const engagementId = String(formData.get("engagementId") ?? "");
  if (!envelopeId) return;
  await releaseContract(envelopeId, admin.id);
  if (engagementId) revalidatePath(`/admin/engagements/${engagementId}`);
}
...
async function autoDraftPhaseDeliverables(milestoneId: string, engagementId: string, actorId: string | null) {
  ...
  for (const d of pending) {
    ...
    // Autonomously drafted → surface in the operator review queue (approvalStatus
    // PENDING) so it "hits the super-admin first" before any client release.
    await prisma.deliverable.update({ where: { id: d.id }, data: { detail, approvalStatus: "PENDING" } });
